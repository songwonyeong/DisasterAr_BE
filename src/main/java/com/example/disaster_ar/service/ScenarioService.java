...
public class ScenarioService {
    ...
    public BeaconDetectResponse detectBeacon(String scenarioId, BeaconDetectRequest req) {
        ScenarioV4 scenario = scenarioRepository.findById(scenarioId)
                .orElseThrow(() -> new IllegalArgumentException("시나리오 없음"));
        ...
        BeaconV4 fromBeacon = student.getLastBeacon();

        StudentBeaconEventV4 beaconEvent = StudentBeaconEventV4.builder()
                .id(UUID.randomUUID().toString())
                .scenario(scenario)
                .student(student)
        ...
        studentBeaconEventRepositoryV4.save(beaconEvent);

        student.setLastBeacon(beacon);
        student.setLastBeaconRssi(req.getRssi());
        student.setLastBeaconSeenAt(now);
        student.setBeaconState(BeaconState.DETECTED);
        studentRepository.save(student);

        List<ScenarioAssignmentV4> assignments =
                scenarioAssignmentRepositoryV4.findByScenario_IdAndBeacon_Id(scenarioId, beacon.getId());

        List<String> triggeredIds = new java.util.ArrayList<>();

        for (ScenarioAssignmentV4 assignment : assignments) {
            boolean alreadyTriggered = scenarioTriggerRepositoryV4
                    .findByScenario_IdAndStudent_IdAndAssignment_Id(
                            scenarioId, student.getId(), assignment.getId()
                    )
                    .isPresent();
        ...
        return BeaconDetectResponse.builder()
